...
! function(e, t) {
	...
	return function(e) {
		...
		function a() {
			function e(t) {
				e = r.default.noop, d.addClass(o ? "weui-animate-fade-out" : "weui-animate-slide-down"), f.addClass("weui-animate-fade-out").on("animationend webkitAnimationEnd", function() {
					u.remove(), s = !1, t && t()
				})
			}
			...
			var u = (0, r.default)(r.default.render(l.default, a)),
				...
				f = u.find(".weui-mask");
			return(0, r.default)("body").append(u), r.default.getStyle(d[0], "transform"), d.addClass(o ? "weui-animate-fade-in" : "weui-animate-slide-up"), f.addClass("weui-animate-fade-in").on("click", function() {
				t()
		...
		function o(e, t, n) {
			...
			if("INPUT" == i.tagName || "TEXTAREA" == i.tagName) {
				...
				if("object" === ("undefined" == typeof d ? "undefined" : s(d))) return d.v
			...
			l.onload = function(l) {
				...
				s.onload = function() {
					var l = n(s),
						...
						m = s.height,
						w = void 0;
					if(v < m && m > h ? (v = parseInt(h * s.width / s.height), m = h) : v >= m && v > p && (m = parseInt(p * s.height / s.width), v = p), f.width = v, f.height = m, d > 0 && r(f, c, d), c.drawImage(s, 0, 0, v, m / l), w = /image\/jpeg/.test(e.type) || /image\/jpg/.test(e.type) ? f.toDataURL("image/jpeg", t.compress.quality) : f.toDataURL(e.type), "file" == t.type)
						if(/;base64,null/.test(w) || /;base64,$/.test(w)) u(e);
						else {
							var _ = a(w);
							_.id = e.id, _.name = e.name, _.lastModified = e.lastModified, _.lastModifiedDate = e.lastModifiedDate, u(_)
						}
					else /;base64,null/.test(w) || /;base64,$/.test(w) ? (t.onError(e, new Error("Compress fail, dataURL is " + w + ".")), u()) : (e.base64 = w, u(e))
		...
		function r() {
			function e() {
				(0, s.default)("body").append(h), s.default.getStyle(h[0], "transform"), h.find(".weui-mask").addClass("weui-animate-fade-in"), h.find(".weui-picker").addClass("weui-animate-slide-up")
			}

			function t(e) {
				t = s.default.noop, h.find(".weui-mask").addClass("weui-animate-fade-out"), h.find(".weui-picker").addClass("weui-animate-slide-down").on("animationend webkitAnimationEnd", function() {
					h.remove(), _ = !1, e && e()
				})
			}
			...
			var a = arguments[arguments.length - 1],
				r = s.default.extend({
					id: "default",
					className: "",
					onChange: s.default.noop,
					onConfirm: s.default.noop
				}, a),
				u = void 0,
				l = !1;
			if(arguments.length > 2) {
				var d = 0;
				for(u = []; d < arguments.length - 1;) u.push(arguments[d++]);
				l = !0
			} else u = arguments[0];
			g[r.id] = g[r.id] || [];
			for(var f = [], c = g[r.id], h = (0, s.default)(s.default.render(v.default, r)), m = a.depth || (l ? u.length : p.depthOf(u[0])), y = ""; m--;) y += w.default;
			return h.find(".weui-picker__bd").html(y), e(), l ? u.forEach(function(e, t) {
				i(e, t)
			}) : i(u, 0), h.on("click", ".weui-mask", function() {
				n()
		...
		o.default.fn.scroll = function(e) {
			...
			function n(e) {
				w = e;
				var t = w - m;
				r(v, 0), u(v, g + t), _ = +new Date, y.push({
					time: _,
					y: w
				}), y.length > 40 && y.shift()
			}
			...
			function i(e) {
				if(m) {
					var t = (new Date).getTime(),
						n = b - c.bodyHeight / 2;
					if(w = e, t - _ > 100) M(Math.abs(w - m) > 10 ? w - m : n - w);
					else if(Math.abs(w - m) > 10) {
			...
			var a = this,
				...
				h = (0, o.default)(this);
			h.find(".weui-picker__content").html(p);
			var v = h.find(c.scrollable),
				m = void 0,
				w = void 0,
				_ = void 0,
				g = void 0,
			...
			if(null !== c.temp && c.temp < c.items.length) {
				var k = c.temp;
				c.onChange.call(this, c.items[k], k), g = (c.offset - k) * c.rowHeight
			} else {
				var x = l(c.items);
				c.onChange.call(this, c.items[x], x), g = s(c.offset, c.rowHeight, c.items)
			}
			u(v, g);
			var M = function(e) {
				g += e, g = Math.round(g / c.rowHeight) * c.rowHeight;
				var t = d(c.offset, c.rowHeight),
					n = f(c.offset, c.rowHeight, c.items.length);
				g > t && (g = t), g < n && (g = n);
				for(var i = c.offset - g / c.rowHeight; c.items[i] && c.items[i].disabled;) e > 0 ? ++i : --i;
				g = (c.offset - i) * c.rowHeight, r(v, .3), u(v, g), c.onChange.call(a, c.items[i], i)
			};
			v = h.offAll().on("touchstart", function(e) {
			...
			var C = "ontouchstart" in window || window.DocumentTouch && document instanceof window.DocumentTouch;
			C || h.on("mousedown", function(e) {
				t(e.pageY), e.stopPropagation(), e.preventDefault()
			}).on("mousemove", function(e) {
				m && (n(e.pageY), e.stopPropagation(), e.preventDefault())
